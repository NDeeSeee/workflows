...
outputs:
  ...
  all_sorted_reads_as_bam_statistics_report:                                 # report
    type: File
  ...
  uniquely_mapped_sorted_deduped_reads_as_bam:                               # data
    type: File
...
# -----------------------------------------------------------------------------------
  ...
  rename_fastqc_report_fastq_1:
    doc: Renames FastQC report for fastq file 1 (technical step)
...
# -----------------------------------------------------------------------------------
  ...
  rename_fastqc_report_fastq_2:
    doc: Renames FastQC report for fastq file 2 (technical step)
    run: ../../tools/rename.cwl
    in:
...
# -----------------------------------------------------------------------------------
  ...
  align_reads:
    ...
    in:
      filelist: trim_adapters/trimmed_file
      filelist_mates: trim_adapters/trimmed_file_pair
      ...
      threads: threads
    out:
      - output                                                        # Unsorted SAM file with both mapped and unmapped reads
      - output_log                                                    # Returned as workflow output. Not used in any other calculations

  sort_and_index:
    doc: |
      Sorts and indexes SAM file. Returns coordinate sorted
      BAM file and BAI index. Includes both mapped and unmapped
      reads
    ...
    in:
      ...
      threads: threads
    out:
    - bam_bai_pair                                                    # Coordinate sorted BAM file and BAI index. Includes both mapped and unmapped reads
...
# -----------------------------------------------------------------------------------
  ...
  filter_reads:
    doc: |
      ...
      BAM file and BAI index. Includes only mapped reads
    run: ../../tools/samtools-filter.cwl
    in:
      bam_bai_pair: sort_and_index/bam_bai_pair                       # Coordinate sorted BAM file and BAI index. Includes both mapped and unmapped reads
    ...
    out:
    - filtered_bam_bai_pair                                           # Coordinate sorted filtered (with chromosomes removed and only mapped reads) BAM file and BAI index

  get_bam_statistics_after_chrom_removal:
    doc: |
      Collects BAM statistics after unused chromosomes and
      umpaped reads removal. We need it to get reads_mapped
      for genome coverage normalization
    run: ../../tools/samtools-stats.cwl
    in:
