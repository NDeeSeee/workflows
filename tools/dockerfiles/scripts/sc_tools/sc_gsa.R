...
export_all_plots <- function(seurat_data, gene_sets, args) {
    ...
    for (i in 1:length(gene_sets)){
        ...
        graphics$vln_plot(
            ...
            plot_title="Gene signature score density per cell group",
            legend_title="Group",
            rotate_labels=TRUE,
            pt_size=0,
...
get_args <- function(){
    ...
    parser$add_argument(
        "--h5ad",
        help="Save raw counts from the GSA assay to h5ad file. Default: false",
        action="store_true"
    )
    parser$add_argument(
        "--cbbuild",
...
if(args$h5ad){
    print("Exporting GSA counts to h5ad file")
    io$export_h5ad(
        data=seurat_data,
