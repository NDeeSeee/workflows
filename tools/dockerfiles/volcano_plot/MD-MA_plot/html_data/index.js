 ...
 * @method maPlot(): generates volcano plot
...
function maPlot() {
    var width = 960,                                                                    
        ...
        yTicks,
        sampleID = "", 
        //significanceThreshold = parseFloat(d3.select("#basemeanthreshold").property("value")), 
        foldChangeThreshold = parseFloat(d3.select("#foldchange").property("value")), 
        colorRange, 
        xScale = d3.scaleLog(), // was scale linear
        yScale = d3.scaleLinear(), // was scale log
        ...
        acceptedColorYpos = d3.select("#acceptedycolorpos").property("value"),
        acceptedColorYneg = d3.select("#acceptedycolorneg").property("value"),

        colorByPvalue = d3.select('#pvaluetoggle-selections-by').property('value'),
        colorFilterPvalue = d3.select('#pvaluetoggle-selections-filter').property('value'),
        pvalColorMin = parseFloat(d3.select('#minpvalue').property('value')),
        pvalColorMax = parseFloat(d3.select('#maxpvalue').property('value')),

        //rejectedColorX = d3.select("#rejectedxcolor").property("value"),
        rejectedColorY = d3.select("#rejectedycolor").property("value"),
        acceptedPcolor = d3.select('#acceptedpvalcolor').property("value"),
        rejectedPcolor = d3.select('#rejectedPvalColor').property("value"),
    ...
    function chart(selection) {
        ...
        selection.each(function (data) {
            ...
            yScale.range([innerHeight, 0])
                .domain([Math.log(d3.quantile(data, 0.01)), Math.log(d3.quantile(data, 1))])//(d3.extent(data, function (d) { return d[yColumn]; }))
            ...
            */
            ...
            var gyLabel = gY.append('text')
                ...
                .html(`(log2) ${yAxisLabel}` || yColumn); // make "log2" inherently part of ONLY the set yAxis label. yColumn already includes "log2"
            ...
            /**
             * Function that defines the html tooltip to be displayed when some data point is hovered
             * @param d individual data point (circle)
             */
            function tipEnter(d) {
                let html;
                if(colorByPvalue != "none"){}
            ...
            /**
             * formats log axis scale for a specific base
             * @param {number} n number to be adjusted for log based displaying 
             * @returns 
             */
            function logBase2Format(n){
                return d3.format(".2r")(getBaseLog(2, n));
            ...
            function circleStyle(d) {
                // SDAP-1056: if pvalue coloring in use, color by pval
                if(colorByPvalue.includes("padj") || colorByPvalue.includes("pvalue")){
                    let colorChoice = colorByPvalue.replace("by: ", "");
                    // check pvalue color filter by Y threshold
                    if(colorFilterPvalue.includes('all')){
                        if(d[colorChoice] > pvalColorMin && d[colorChoice] < pvalColorMax){
                            // distinguish p value color by Y threshold
                            if(Math.abs(d[yColumn]) >= foldChangeThreshold){
                                return acceptedPcolor;
                            }
                            else{return rejectedPcolor;}
                        }
                    }
                    else if(colorFilterPvalue.includes('accepted')){
                        if(Math.abs(d[yColumn]) >= foldChangeThreshold &&
                        d[colorChoice] > pvalColorMin && d[colorChoice] < pvalColorMax){
                            return acceptedPcolor
                        }
                    }
                    else if(colorFilterPvalue.includes('rejected')){
                        if(Math.abs(d[yColumn]) < foldChangeThreshold &&
                        d[colorChoice] > pvalColorMin && d[colorChoice] < pvalColorMax){
                            return rejectedPcolor;
                        }
                    }
                }

                /* SDAP-1032 MA-plot
                 need to swap comparison so fold-change is Y-axis, and significance is X-axis
                if(d[yColumn] <= significanceThreshold && d[xColumn] >= foldChangeThreshold) return acceptedColorYpos;
                else if(d[yColumn] <= significanceThreshold && Math.abs(d[xColumn]) >= foldChangeThreshold) return acceptedColorYneg;
                ...
                */
                // attempting to recreate default coloring according to image at https://github.com/alisheharyar/Enhanced_MA_Plot
                if( d[yColumn] <= -1*foldChangeThreshold /*&& d[xColumn] <= significanceThreshold*/) return acceptedColorYneg;
                else if( d[yColumn] >= foldChangeThreshold /*&& d[xColumn] <= significanceThreshold*/) return acceptedColorYpos;
                // else if (Math.abs(d[yColumn]) >= foldChangeThreshold && d[xColumn] >= significanceThreshold) return rejectedColorX; //"#ff0000"//'red';
                // else if (Math.abs(d[yColumn]) <= foldChangeThreshold && d[xColumn] <= significanceThreshold) return rejectedColorY; //'#cccccc';
                // else return rejectedColorBoth; //'#000000';
                return rejectedColorY
            }
            ...
            /** to be used by tick formatter on x axis */
            ...
            function updatePlot() {
                ...
                if(customAxisX === 'Custom'){
                    ...
                    let xRange = d3.extent(data, function (d) { return d[xColumn]; });
                    console.log('xRange: ', xRange);
                    let newXmax  = xRange[1];
                    let newXmin = xRange[0];
                    // if log axis min is 0, set to next min
                    if(newXmin == 0 || newXmin == '0'){
                        newXmin = d3.min(data, d => d[xColumn] != '0' ? d[xColumn] / 2 : NaN);
                        console.log(`invalid log scaled min. setting to 1/10 of actual min instead:  ${newXmin}`);
                    }
                ...
                if(customAxisY === 'Custom'){
                    ...
                    let newYmin;
                    let parsedYminInt = parseFloat(d3.select('#customyrangemin').property('value'));
                    /*if(parsedYminInt<0){
                       newYmin = parseFloat(`-1e${-1*parsedYminInt}`);
                    }else{
                       newYmin = parseFloat(`1e${parsedYminInt}`);
                    }*/
                    newYmin = parsedYminInt;
                    let newYmax = parseFloat(d3.select('#customyrangemax').property('value'));//parseFloat(`1e${d3.select('#customyrangemax').property('value')}`); // was previously hardcoded to be 1
                    console.log('new y max: ', newYmax)
                ...
                // *** handle when coloring by pvalue (requires sorting data so dots aren't covered)
                /*
                if(colorByPvalue === "pvalue"){
                    data = data.sort((a, b) => {
                        let aval = a['pvalue'], bval = b['pvalue'];

                        // if a padj is outside desired range, push to front
                        if(aval >  pvalColorMax || aval < pvalColorMin){
                            return 1; 
                        }
                        // if b padj is outside desired range, push to front
                        if(bval >  pvalColorMax || bval < pvalColorMin){
                            return 1; 
                        }

                        // if neither are outside range, sort by pval
                        return aval - bval;
                    });
                }
                else if(colorByPvalue === "padj"){
                    data = data.sort((a, b) => {
                        let aval = a['padj'], bval = b['padj'];
                ...
                */
                ...
                // console.log('type of yScale.domain[0]: ', typeof yScale.domain()[0], yScale.domain()[0])
                // console.log('type of yScale.domain[1]: ', typeof yScale.domain()[1], yScale.domain()[1])
                // console.log('type of xScale.domain[0]: ', typeof xScale.domain()[0], xScale.domain()[0])
                // console.log('type of xScale.domain[1]: ', typeof xScale.domain()[1], xScale.domain()[1])

                // console.log('y col name: ', yColumn);
                // console.log('x col name: ', xColumn);




                // after axis scaling is set, adjust threshold values

                // format significance (base-mean) threshold to account for log2 entered data, that needs to be log10 for placement on plot
                // console.log('base-mean threshold BEFORE log2 formatting: ', significanceThreshold);
                // //significanceThreshold = Math.log2(significanceThreshold) / Math.log2(10);
                // significanceThreshold = 2**significanceThreshold;
                // console.log('base-mean threshold AFTER log2 formatting: ', significanceThreshold);

                // ************* redraw the plot *************
                xAxis = d3.axisBottom(xScale)
                    //.ticks(10, "s");//formatPower);
                    .ticks(5)
                    .tickFormat(logBase2Format);

                yAxis = d3.axisLeft(yScale)
                    .ticks(5) // can be exported as custom option in advanced plot details
                    //.tickFormat(yTickFormat);
                ...
                let yLabelText = yAxisLabel ? `(log2) ${yAxisLabel}` : yColumn; // ternary operator to add "log2" to ONLY axisLabel (y column has "log2" in name already)
                ...
                //     .attr("x1", xScale(significanceThreshold))
                //     .attr("x2", xScale(significanceThreshold))
                //     .attr("y1", 0)
                //     .attr("y2", innerHeight);

                // add horizontal line(s) at fold-change threshold (and negative fold-change)
                ...
                pvalueMap = pvalueMap.filter((dataPoint) => {
                    // adjust for different plot ...
                    let thresholdStatus;
                    if(dataPoint.yVal >= foldChangeThreshold){thresholdStatus = "pos";}
                    else if(dataPoint.yVal <= -1*foldChangeThreshold){thresholdStatus = 'neg'}
                    else{thresholdStatus = "fail"}

                    if(highlightTopXFilter.includes('all')){
                        return true;
                    }
                    else if(highlightTopXFilter.includes('both') && thresholdStatus !== "fail"){
                        return true;
                    }
...
function createForm(formDivId){
    ...
    var webFormContent = {
        ...
        main: {
            ...
            htmlId: 'mainform',
            formControls: {
                order: ['title', 'ylabel', 'xlabel', 'foldchangethreshold',
                        //, 'basemeanthreshold'],
                        'pvalToggle', 'minpvalue', 'maxpvalue',
                    ],
                title: {
                    label: 'Plot Title: ',
                    inputType: 'text',
                ...
                ylabel: {
                    label: 'Y Axis Label: ',
                    inputType: 'text',
                    value: 'Fold-Change',//'-log<tspan baseline-shift="sub">10</tspan>False Discovery Rate',
                    htmlId: 'plotyaxis',
                },
                xlabel: {
                    label: 'X Axis Label: ',
                    inputType: 'text',
                    value: 'Base Mean',//'log<tspan baseline-shift="sub">2</tspan>Fold-change',
                    htmlId: 'plotxaxis',
                },
                /*basemeanthreshold: {
                    label: 'log2 Base Mean (X) threshold: ',
                    inputType: 'text',
                    value: '5',
                    htmlId: 'basemeanthreshold',
                },*/
                foldchangethreshold: {
                    label: 'log2 Fold-change (Y) threshold: ',
                    inputType: 'text',
                    value: '1',
                    htmlId: 'foldchange',
                },
                ...
                //SDAP-1056
                pvalToggle: {
                    label: "Color by p value (overwrites other coloring options): ", 
                    inputType: 'none',
                    htmlId: 'pvaluetoggle',
                    extraAddons: {
                        options: [
                            {
                                ...
                                selectorID: "by",
                                type: "single-select", 
                                options: ["pvalue", "padj", "none"]
                            },
                            {
                                label: "include: ",
                                selectorID: "filter",
                                type: "single-select", 
                                options: ["all", "accepted Y threshold ", "rejected Y threshold"]
                            }
                        ]
                    }
                },
                minpvalue: {
                    label: 'Min pvalue to color: ',
                    inputType: 'number',
                    value: '0',
                    htmlId: 'minpvalue',
                    extraAddons: {
        ...
        advanced: {
            ...
            formControls: {
                ...
                customXminRange: {
                    label: "(custom) x-axis (log2) min:", 
                    inputType: 'number',
                    value: "0.00001",
                    htmlId: 'customxrangemin',
                },
                customXmaxRange: {
                    label: "(custom) x-axis (log2) max:", 
                    inputType: 'number',
                    value: "10",
                ...
                customYminRange: {
                    label: "y-axis (log2) min", 
                    inputType: 'number',
                    value: "-57",
                    htmlId: 'customyrangemin',
                },
                customYmaxRange: {
                    label: "y-axis (log2) max (should always be 1)", 
                    inputType: 'number',
                    value: "1",
        ...
        color: {
            ...
            htmlId: 'colorform',
            formControls: {
                order: ['plotdotopacity', 'plotdotsize', 'acceptedycolorpos', 'acceptedycolorneg', 'rejectedycolor', 'acceptedPvalColor', 'rejectedPvalColor'],//, 'rejectedxcolor', 'rejectedbothcolor'],  
                plotdotopacity: {
                ...
                acceptedycolorpos: {
                    label: 'Accepted (positive Y) color: ',
                    inputType: 'color',
                    value: '#00ff00',
                    htmlId: 'acceptedycolorpos'
                },
                acceptedycolorneg: {
                    label: 'Accepted (negative Y) color: ',
                    inputType: 'color',
                    value: '#00ff00',
                    htmlId: 'acceptedycolorneg'
                },
                /*
                rejectedxcolor: {
                ...
                */
                acceptedPvalColor: {
                    label: 'p-val color (overwrites accepted Y threshold) : ',
                    inputType: 'color',
                    value: '#0000aa',
                    htmlId: 'acceptedpvalcolor'
                },
                rejectedPvalColor: {
                    label: 'p-val color (overwrites rejected Y threshold): ',
                    inputType: 'color',
                    value: '#9900aa',
                    htmlId: 'rejectedPvalColor'
                },
            }
        },
        highlight: {
            ...
            formControls: {
                ...
                /*highlighttopn: {
                    label: 'Highlight top (N) Features: ',
                    inputType: 'number',
                    value: '0',
                    ...
                    extraAddons: {
                        ...
                        options: [
                            ...
                            },
                            {
                                ...
                                selectorID: "filter",
                                type: "single-select", 
                                options: ["all", "positive of Y threshold only", "negative of Y threshold only", "both (pos/neg) of Y threshold", "neither (pos/neg) of Y threshold"]
                            }
                        ]
                    }
                },*/
                genehighlight: {
                    label: 'Enter feature name(s) (click name on right): ',
                    inputType: 'text',
                    value: '',
                    htmlId: 'q',
                    extraAddons: {
                ...
                selectedGeneList: {
                    label: "Selected Features to highlight: ",
